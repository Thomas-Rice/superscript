...
class Container(QtGui.QWidget):
	...
	def create_connections(self):
		self.button.clicked.connect(self.return_configs)
	...
	def make_final_dictionary(self,final_dictionary):
		final_build = []
		final_config = [] 
		# self.return_configs()
		...
		for element in final_dictionary:
			for i in range(len(element[1])):
				tmp = []
				print original_dict['Modo']['Joota_901dev']
				# final_build.append(element)
				# for j in original_dict[element[0][0]][element[1][i]]:
				# 	# print j
				# 	tmp.append(j)
				# final_config.append(tmp)

		# for t in range(len(final_build)):
		# 	print final_build[t]
...
class main_window(QtGui.QWidget):
	...
	def change_Layout(self):
		if self.layout == "First_Page":
			...
			self.product_form.return_checked()
	...
	def generate_config_window(self):
		...
		self.config_checkbox_form = Container(product_and_config[0],product_and_config[1])
